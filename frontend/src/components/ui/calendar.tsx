...
import { ShugarInput } from "@/components/ui/shugar-input"
...
const ArrowBottomIcon = ({ className }: { className?: string }) => (
  <svg
    ...
    viewBox="0 0 16 16"
    width="16"
    className={clsx("fill-slate-900", className)}
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
...
const parseFixedRange = (input: string) => {
  ...
  for (const formatPattern of possibleFormats) {
    ...
    const start = parse(startStr, formatPattern, now, { locale: enUS })
    const end = parse(endStr, formatPattern, now, { locale: enUS })

    const finalStart = isValid(start) ? startOfDay(start) : null
    const finalEnd = isValid(end) ? endOfDay(end) : null

    if (finalStart && finalEnd) {
      if (formatPattern === "d MMM") {
...
const parseExactDate = (input: string) => {
  ...
  for (const formatPattern of dateFormats) {
    const date = parse(input.trim(), formatPattern, now, { locale: enUS })
  ...
  return (
    ...
    >
      <ShugarInput
        ...
        className={clsx(
          "pl-2 placeholder:!text-slate-900 placeholder:!opacity-100",
          compact &&
            !isOpen &&
            (!currentPreset ||
              (currentPreset?.start !== value?.start && currentPreset?.end !== value?.end)) &&
      ...
      <Material
        type="menu"
        className={clsx(
          "absolute z-50 top-12 left-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800",
          compact ? "w-full" : "grid grid-cols-2 w-[200%]",
          isOpen && "opacity-100",
          !isOpen && "opacity-0 pointer-events-none duration-200"
        )}
      >
        <ul className="p-2 border-r border-slate-200 dark:border-slate-800">
          {Object.entries(filteredPresets).length > 0 ? (
            Object.entries(filteredPresets).map(([key, preset]) => (
              <li
                key={key}
                className="flex items-center cursor-pointer px-2 w-full h-9 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 font-sans text-sm text-slate-900"
            ...
            <li className="flex items-center px-2 w-full h-9 rounded-md font-sans text-sm text-gray-500">
              {inputValue}
            </li>
          )}
        </ul>
          ...
          <div className="p-4 pr-[30px]">
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              {typeRelativeTimes.map((preset) => (
                <button
                  key={preset.text}
                  className="font-mono text-[13px] text-gray-900 px-1.5 h-5 inline-flex items-center bg-slate-100 border border-transparent rounded hover:bg-slate-200"
                  onClick={() => onClick(preset)}
                >
                  {preset.text}
                </button>
...
export const Calendar = ({
  ...
  const [startDate, setStartDate] = useState<string>(format(value?.start || new Date(), "MMM dd, yyyy"))
  const [startTime, setStartTime] = useState<string>(format(startOfDay(value?.start || new Date()), "HH:mm"))
  const [endDate, setEndDate] = useState<string>(format(value?.end || new Date(), "MMM dd, yyyy"))
  const [endTime, setEndTime] = useState<string>(format(endOfDay(value?.end || new Date()), "HH:mm"))
  ...
  const handleMouseEnter = (hoveredDay: Date) => {
    if (value?.start && !value.end) {
      setHoverDate(hoveredDay)
    }
  }
  ...
  useEffect(() => {
    setStartDate(format(value?.start || new Date(), "MMM dd, yyyy"))
    setStartTime(format(value?.start || startOfDay(new Date()), "HH:mm"))
    setEndDate(format(value?.end || new Date(), "MMM dd, yyyy"))
    setEndTime(format(value?.end || endOfDay(new Date()), "HH:mm"))
  }, [isOpen, value])
      ...
      >
        ...
        <div className="flex justify-between items-center">
          <div className="relative w-full">
            <Button
              className={clsx(
                "!justify-start focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background",
                presets && !stacked && !compact && "rounded-l-none sm:-ml-px",
                presets && stacked && !compact && "rounded-t-none sm:-mt-px",
                presets && compact && "rounded-r-none -mr-px",
                compact ? "w-full gap-1.5" : "w-full"
              )}
              prefix={<CalendarIcon />}
              type="secondary"
            ...
            >
              <div className="truncate pr-4">
                {value?.start && value?.end ? formatDateRange(value.start, value.end) : "Select Date Range"}
              </div>
            </Button>
            {allowClear && value?.start && value?.end && (
              <Button
                ...
                svgOnly
                variant="unstyled"
                className="absolute right-2 top-1/2 -translate-y-1/2 fill-slate-500 hover:fill-slate-900"
          ...
          <div className={clsx(horizontalLayout && "flex gap-5")}>
            <div>
              ...
              <div className="grid grid-cols-7 items-center gap-y-2">
                {daysArray.map((dayItem) => {
                  const isStart = value?.start && isSameDay(dayItem, value.start)
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={() => isAllowedDate && handleMouseEnter(dayItem)}
                      onClick={() => isAllowedDate && handleDateClick(dayItem)}
                    >
                      <div
                        className={clsx(
            ...
            <div
              className={clsx(
                ...
                horizontalLayout
                  ? "justify-between"
                  : "mt-3 -mx-3 px-3 pt-2.5 border-t border-slate-200 dark:border-slate-800"
              )}
            >
